...
export default function WorkerDetails() {
  ...
  // Logic for ILP/Membership Expiry
  const expiryDate = new Date(userData.worker_profile.ilp_expiry_date);
  const today = new Date();
  const daysLeft = Math.max(0, Math.ceil((expiryDate.getTime() - today.getTime()) / (1000 * 3600 * 24)));
    ...
    <ScrollView style={styles.container} showsVerticalScrollIndicator={false}>
      ...
      <View style={styles.headerCard}>
        <View style={styles.profileRow}>
          <View style={styles.avatarWrapper}>
            <Image 
              source={{ uri: `https://ui-avatars.com/api/?name=${userData.name}&background=0D8ABC&color=fff` }} 
              style={styles.avatar} 
            />
          ...
          <View style={styles.nameContainer}>
            <Text style={styles.userName}>{userData.worker_profile.full_name}</Text>
            <Text style={styles.userRole}>{userData.role_name.toUpperCase()}</Text>
            <View style={styles.locationTag}>
              <Feather name="map-pin" size={12} color="#64748b" />
              <Text style={styles.locationText}>{userData.worker_profile.city.name}, {userData.worker_profile.permanent_pincode}</Text>
            </View>
          </View>
        </View>
      </View>
      ...
      <View style={styles.validityCard}>
        <View style={styles.validityHeader}>
          ...
          <View style={[styles.statusTag, { backgroundColor: isLowTime ? "#fff1f2" : "#f0fdf4" }]}>
            <Text style={[styles.statusTagText, { color: isLowTime ? "#e11d48" : "#16a34a" }]}>
              {isLowTime ? "Renew Soon" : "Active"}
            </Text>
          </View>
        </View>
        
        <Text style={[styles.daysText, { color: isLowTime ? "#e11d48" : "#1e293b" }]}>
          {daysLeft} <Text style={styles.daysSub}>Days Remaining</Text>
        </Text>

        <View style={styles.progressContainer}>
          <View style={[styles.progressBar, { width: `${progress}%`, backgroundColor: isLowTime ? "#e11d48" : "#2563eb" }]} />
        </View>
      </View>
      ...
      <View style={styles.addressCard}>
        ...
        <View style={styles.addressRight}>
          ...
          <Text style={styles.addressValue}>{userData.worker_profile.present_address}</Text>
        </View>
      </View>
...
const styles = StyleSheet.create({
  ...
  avatar: { width: 85, height: 85, borderRadius: 30, backgroundColor: "#e2e8f0" },
  verifiedBadge: { position: "absolute", right: -5, bottom: -5, backgroundColor: "#fff", borderRadius: 12, padding: 2 },
  nameContainer: { marginLeft: 20, flex: 1 },
  userName: { fontSize: 24, fontWeight: "800", color: "#1e293b" },
  userRole: { fontSize: 12, fontWeight: "700", color: "#64748b", letterSpacing: 1, marginVertical: 4 },
  locationTag: { flexDirection: "row", alignItems: "center", gap: 4 },
  locationText: { fontSize: 13, color: "#64748b" },
  validityCard: { backgroundColor: "#fff", padding: 20, borderRadius: 24, shadowColor: "#000", shadowOpacity: 0.05, shadowRadius: 10, elevation: 2, marginBottom: 25 },
  validityHeader: { flexDirection: "row", justifyContent: "space-between", alignItems: "center" },
  ...
  sectionLabel: { fontSize: 16, fontWeight: "700", color: "#1e293b", marginBottom: 15 },
  infoGrid: { flexDirection: "row", flexWrap: "wrap", justifyContent: "space-between", gap: 12 },
  tile: { width: (width - 52) / 2, backgroundColor: "#fff", padding: 15, borderRadius: 20, borderWidth: 1, borderColor: "#f1f5f9" },
  tileIcon: { width: 32, height: 32, borderRadius: 10, backgroundColor: "#eff6ff", justifyContent: "center", alignItems: "center", marginBottom: 10 },
  tileLabel: { fontSize: 11, color: "#94a3b8", fontWeight: "600", textTransform: "uppercase" },
  tileValue: { fontSize: 14, fontWeight: "700", color: "#334155", marginTop: 2 },
  addressCard: { flexDirection: "row", backgroundColor: "#fff", padding: 15, borderRadius: 20, marginTop: 20, alignItems: "center", gap: 15 },
  addressRight: { flex: 1 },
  ...
  logoutBtn: { flexDirection: "row", alignItems: "center", justifyContent: "center", height: 56, borderRadius: 16, borderWidth: 1, borderColor: "#fee2e2", gap: 10 },
